...
void DigiDollarWidgetTests::digiDollarAmountLabelsUseCurrencyPrefix()
{
    ...
    if (nativeDialogsAvailable) {
        ...
        QTimer::singleShot(0, [&] {
            for (QWidget* widget : QApplication::topLevelWidgets()) {
                ...
                // Static QMessageBox helpers return clickedButton(), so done(Yes)
                // alone does not authorize the action.
    ...
    sendWidget.findChild<PaymasterSendWidget*>()->setPaymasterSessionForTesting(
