...
RPCHelpMan senddigidollar()
{
    return RPCHelpMan{"senddigidollar",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            const auto paymaster_options = wallet::ParsePaymasterSendOptions(request);
            ...
            if (paymaster_options.fee_mode == DigiDollar::Paymaster::FeeMode::DGB && pwallet->IsLocked()) {
                throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED,
            ...
            if (paymaster_options.send_all_spendable_dd && preset_dd_inputs) {
                throw JSONRPCError(
            ...
            LogPrint(BCLog::DIGIDOLLAR, "DigiDollar RPC: Parsed send parameters\n");
            ...
            if (auto paymaster_result = wallet::PrepareDigiDollarFeeFunding(
                    request, *pwallet, *dd_wallet, addressStr, dd_address, amount,
                    paymaster_options, selected_inputs, preset_dd_inputs)) {
                return std::move(*paymaster_result);
            }
